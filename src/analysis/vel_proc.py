...
from src.utils.helper_utils import is_interleaved
...
def vv_to_fac(vv, fac_matrix):
    ...
    """
    n_sweeps = vv.shape[2] # Get the number of sweeps: 1 or 2 or for all time.

    # 1. Match rotation matrix time indices to the number of sweeps
    if n_sweeps <= 2:
        # Use first 1 or 2 matrices for mean-phi/interleaved cases
        rotation_to_use = fac_matrix[:n_sweeps, :, :] 
    else:
        # Match full time-series
        rotation_to_use = fac_matrix

    # 2. Batch Rotation via Einstein Summation
    vv_fac = np.einsum('tij, jbt -> ibt', rotation_to_use, vv)
